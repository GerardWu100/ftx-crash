...
def write_btc_spot_parquet() -> None:
    ...
    data = data.loc[(data["DateTime"] >= BTC_START) & (data["DateTime"] <= BTC_END)].copy()
...
def write_bito_parquet() -> None:
    ...
    data = data.loc[(data["DateTime"] >= BITO_START) & (data["DateTime"] <= BITO_END)].copy()
    data.to_parquet(target_path, index=False)
    print(f"Wrote {target_path.name}: {len(data):,} rows")
...
def write_perpetual_parquet() -> None:
    ...
    source_path = RAW_DATA_DIR / "DA-16_BTC_USDT_OPEN_HIGH_LOW_LAST_PRICE_hourly_2019-2025_binance_perpetual.csv"
    target_path = PROCESSED_DATA_DIR / "DA-16_BTC_USDT_OPEN_HIGH_LOW_LAST_PRICE_hourly_2019-2025_binance_perpetual.parquet"

    data = pd.read_csv(source_path)
    data["datetime"] = pd.to_datetime(data["datetime"])
    data = data.loc[(data["datetime"] >= PERP_START) & (data["datetime"] <= PERP_END)].copy()
    data.to_parquet(target_path, index=False)
    print(f"Wrote {target_path.name}: {len(data):,} rows")
