...
f_jun = pd.read_parquet(
    old_data_dir
    / "DA-16_BTC_USDT_OPEN_HIGH_LOW_LAST_PRICE_hourly_2022-03_2022-06_binance_quarterly.parquet"
)
f_sep = pd.read_parquet(
    old_data_dir
    / "DA-16_BTC_USDT_OPEN_HIGH_LOW_LAST_PRICE_hourly_2022-06_2022-09_binance_quarterly.parquet"
)

for futures_df in [f_jun, f_sep]:
    futures_df["datetime"] = pd.to_datetime(futures_df["datetime"]).dt.tz_localize(
        "UTC"
    )
