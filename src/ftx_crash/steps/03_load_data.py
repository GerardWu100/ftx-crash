...
# Step 06 needs this proxy to build the FTX basis, so stop here rather than
# failing later with an unrelated error about a missing attribute.
if not os.path.exists(bito_path):
    raise FileNotFoundError(
        f"BITO futures proxy not found at {bito_path}; "
        "rebuild it with scripts/convert_raw_to_parquet.py"
    )
